...
struct sGraphAuxiliaryData *CreateGraphAuxiliaryData(char *filename){
  struct sGraphAuxiliaryData *GAD;
  int s,t,i;
  int nne, nedges, e, nnodes;
  int phi_0, phi_1, phi_00, phi_01, phi_10, phi_11;
  FILE *fp;
  long pos;
  ...
  for(i = 0; i < nnodes; i++){
    fscanf(fp,"%d %d %d", &s, &phi_0, &phi_1);
    if(s < 0 || s >= nnodes){
      printf("Error: invalid node in CreateGraphAuxiliaryData\n");
      exit(1);
    }
  }
  ...
  //counting the number of neighbors per node:
  pos = ftell(fp);
  for(e = 0; e < nedges; e++){
    fscanf(fp,"%d %d %d %d %d %d", &s, &t, &phi_00, &phi_01, &phi_10, &phi_11);
    GAD->NE.nnedges[s] += 1;
    GAD->NE.nnedges[t] += 1;
  }

  for(s = 0; s < nnodes; s++){
    GAD->NE.N[s] = (int *)calloc(GAD->NE.nnedges[s], sizeof(int));
    if(GAD->NE.N[s] == NULL){
      printf("Error: CreateGraphAuxiliaryData\n");
      exit(1);
    }
...
struct sArrayAtoms *CreateOrderedArrayAtoms(char *filename,
					    struct sGraphAuxiliaryData *GAD){
  struct sArrayAtoms *AA = NULL, *AA_sorted = NULL;
  glip::sPQueue32 *Q=NULL;
  int a,s,t,e,i,j,k,w,nnodes,nedges,phi_max;
  int phi_0, phi_1, phi_00, phi_01, phi_10, phi_11;
  FILE *fp;
...
//=============OIFT=========================
glip::sGraph *graph_construction_MaxNorm(char *filename){
  glip::sGraph *graph;
  int i,e,s,t,nnodes,nedges,node_s,node_t;
  int phi_0, phi_1, phi_00, phi_01, phi_10, phi_11;
  FILE *fp;
  ...
  fscanf(fp,"%d", &nnodes);
  graph = glip::Graph::Create(nnodes+2, 10, NULL);
  node_s = nnodes;
  node_t = nnodes+1;
  for(i = 0; i < nnodes; i++){
    fscanf(fp,"%d %d %d", &s, &phi_0, &phi_1);
    ...
    glip::Graph::AddDirectedEdge(graph, node_s, s, phi_0);
    glip::Graph::AddDirectedEdge(graph, s, node_s,     0);
    
    glip::Graph::AddDirectedEdge(graph, s, node_t, phi_1);
    glip::Graph::AddDirectedEdge(graph, node_t, s,     0);
  }

  fscanf(fp,"%d", &nedges);
  for(e = 0; e < nedges; e++){
    fscanf(fp,"%d %d %d %d %d %d", &s, &t, &phi_00, &phi_01, &phi_10, &phi_11);
    glip::Graph::AddDirectedEdge(graph, s, t, phi_10);
    glip::Graph::AddDirectedEdge(graph, t, s, phi_01);
    if(phi_01 >= phi_10){
      glip::Graph::UpdateDirectedEdgeIfHigher(graph, node_s, s, phi_00);
      glip::Graph::UpdateDirectedEdgeIfHigher(graph, t, node_t, phi_11);      
    }
    else{
      glip::Graph::UpdateDirectedEdgeIfHigher(graph, node_s, t, phi_00);
      glip::Graph::UpdateDirectedEdgeIfHigher(graph, s, node_t, phi_11);
    }
  }
  fclose(fp);
  return graph;
...
int *Segmentation_OIFT_MinMax_1st(char *filename, int *nnodes){
  glip::sGraph *graph;
  int S_bkg[2];
  int S_obj[2];
  int *label;
  int n;
  graph = graph_construction_MaxNorm(filename);
  n = graph->nnodes;
  S_bkg[0] = 1; S_bkg[1] = n-1;
  S_obj[0] = 1; S_obj[1] = n-2;
  label = Segmentation_OIFT_MinMax_1st(graph, S_bkg, S_obj);
#ifdef APPDEBUG
  int energy;
  energy = glip::ift::GetEnergy_Max(graph, label, 1);
  printf("energy: %d\n", energy);
#endif
  glip::Graph::Destroy(&graph);
  *nnodes = n-2;
  return label;
}


int *Segmentation_OIFT_MinMax_2nd(char *filename, int *nnodes){
  glip::sGraph *graph;
  int S_bkg[2];
  int S_obj[2];
  int *label;
  ...
  S_bkg[0] = 1; S_bkg[1] = n-1;
  S_obj[0] = 1; S_obj[1] = n-2;
  label = Segmentation_OIFT_MinMax_2nd(graph, S_bkg, S_obj);
#ifdef APPDEBUG
  int energy;
  energy = glip::ift::GetEnergy_Max(graph, label, 1);
  printf("energy: %d\n", energy);
...
int *Segmentation_ORFC_MinMax(char *filename, int *nnodes){
  glip::sGraph *graph;
  int S_bkg[2];
  int S_obj[2];
  int *label;
  ...
  S_bkg[0] = 1; S_bkg[1] = n-1;
  S_obj[0] = 1; S_obj[1] = n-2;
  label = Segmentation_ORFC_MinMax(graph, S_bkg, S_obj);
#ifdef APPDEBUG
  int energy;
  energy = glip::ift::GetEnergy_Max(graph, label, 1);
  printf("energy: %d\n", energy);
...
int main(int argc, char **argv){
  ...
  if(argc < 2){
    ...
    fprintf(stdout,"OptMaxNorm_graph <graph>\n");
    fprintf(stdout,"graph .... text file of the graph with phi penalties (.txt),\n");
    exit(0);
  }
  ...
  //------------------------------------------------
  GAD = CreateGraphAuxiliaryData(filename);

  H = CreateOrderedArrayAtoms(filename, GAD);
  ...
  label = Segmentation_OIFT_MinMax_1st(filename, &nnodes);
  ...
  label = Segmentation_OIFT_MinMax_2nd(filename, &nnodes);
  ...
  label = Segmentation_ORFC_MinMax(filename, &nnodes);
