...
namespace glip{
  namespace GQueue{
    ...
    sGQueue *Create(int nbuckets, int nelems, int *value){
      ...
      Q = (sGQueue *) malloc(1*sizeof(sGQueue));
	  ...
	  Q->L.elem = (sGQNode *)malloc(nelems*sizeof(sGQNode));
	  Q->L.nelems = nelems;
	  Q->L.value   = value;
	  if (Q->L.elem != NULL){
	    Reset(Q);
	    ...
	    glip::Error((char *)MSG1,(char *)"GQueue::Create");
        }
        else
	  glip::Error((char *)MSG1,(char *)"GQueue::Create");
      }
    ...
    sGQueue *Grow(sGQueue **Q, int nbuckets){
      sGQueue *Q1=Create(nbuckets,(*Q)->L.nelems,(*Q)->L.value);
      int i,bucket;
    ...
    void Insert(sGQueue **Q, int elem){
	  ...
	  (*Q) = Grow(Q,2*(maxvalue-minvalue)+1);
    ...
    int Remove(sGQueue *Q){
	  ...
	  if (Q->C.first[Q->C.nbuckets] != NIL){
	    ...
	    glip::Error((char *)"GQueue is empty\n",(char *)"GQueue::Remove");
	  }
        }
      }
