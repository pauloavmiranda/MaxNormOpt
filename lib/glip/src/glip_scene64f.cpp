...
namespace glip{
  ...
  sScene64f *Create(int xsize,int ysize,int zsize){
    ...
    scn = (sScene64f *) calloc(1,sizeof(sScene64f));
    if(scn == NULL) 
      glip::Error((char *)MSG1,(char *)"Scene64f::Create");
    ...
    scn->data   = glip::AllocDoubleArray(N);
    if(scn->data==NULL) 
      glip::Error((char *)MSG1,(char *)"Scene64f::Create");

    scn->array = (double ***) calloc(zsize, sizeof(double **));
    if(scn->array==NULL) 
      glip::Error((char *)MSG1,(char *)"Scene64f::Create");

    tmp = (double **) calloc(zsize*ysize, sizeof(double *));
    if(tmp==NULL) 
      glip::Error((char *)MSG1,(char *)"Scene64f::Create");
  ...
  void     Destroy(sScene64f **scn){
    ...
    aux = *scn;
    if(aux != NULL){
      if(aux->data     != NULL) glip::FreeDoubleArray(&aux->data);
  ...
  sScene64f *Clone(sScene64f *scn){
    sScene64f *aux = Create(scn->xsize,scn->ysize,scn->zsize);
    aux->dx     = scn->dx;
    aux->dy     = scn->dy;
    aux->dz     = scn->dz;
  ...
  void     Copy(sScene64f *dest, sScene64f *src){
    if(dest->xsize!=src->xsize ||
      ...
      glip::Error((char *)"Incompatible data",(char *)"Scene64f::Copy");
    dest->dx     = src->dx;
    dest->dy     = src->dy;
    dest->dz     = src->dz;
  ...
  void     Write(sScene64f *scn, char *filename){
    ...
    if(fp == NULL) 
      glip::Error((char *)MSG2,(char *)"Scene64f::Write");
