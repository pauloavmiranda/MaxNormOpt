...
namespace glip{
  namespace PQueue32{
    ...
    sPQueue32 *Create(int nbuckets, int nelems, int *value){
      ...
      Q = (sPQueue32 *) malloc(1*sizeof(sPQueue32));
	...
	if ( (Q->C.first != NULL) && (Q->C.last != NULL) ){
	  Q->L.elem = (sPQNode *)malloc(nelems*sizeof(sPQNode));
	  Q->L.nelems = nelems;
	  Q->L.value  = value;
	  if (Q->L.elem != NULL){
	    Reset(Q);
	...
	*Q = NULL;
    ...
    size_t GetSizeOf(sPQueue32 *Q){
      size_t size = 0;
      if(Q == NULL)
	return 0;
      size = sizeof(sPQueue32);
      size += (Q->C.nbuckets+1)*sizeof(int);
      size += (Q->C.nbuckets+1)*sizeof(int);
      size += Q->L.nelems*sizeof(sPQNode);
      //printf("%zu\n", size);
      return size;
    }
	...
	if (dvalue > ((*Q)->C.nbuckets-1)){
	  (*Q) = Grow(Q, 2*(dvalue)+1);
	  glip::Warning((char *)"Doubling queue size",
    ...
    /// \cond
    inline int _FindMinBucket(sPQueue32 *Q){
      int current,last;
	...
	if(Q->C.first[current] != NIL)
	  ...
	  if(Q->C.first[Q->C.nbuckets] != NIL){
	    ...
	    glip::Error((char *)"PQueue is empty",
		       (char *)"_FindMinBucket");
	}
      }
      return current;
    ...
    /// \cond    
    inline int _FindMaxBucket(sPQueue32 *Q){
      int current,last;
    ...
    /// \cond
    inline int _BucketFIFO(sPQueue32 *Q, int bucket){
      int elem=NIL, next;
    ...
    /// \cond
    inline int _BucketLIFO(sPQueue32 *Q, int bucket){
      int elem=NIL, prev;
    ...
    void FastUpdateElem(sPQueue32 *Q, int elem, int newvalue){
      ...
      /*
      if(Q->L.elem[elem].color != GRAY){
	printf("Elemento não estava na fila: %d\n",elem);
      }
      */
      //================================
...
#ifdef GLIP_CHECK_ALL_ERRORS
    /// \cond
    inline int _FastFindMinBucket(sPQueue32 *Q){
      int current;
    ...
    /// \endcond
#endif
#ifndef GLIP_CHECK_ALL_ERRORS
    inline int _FastFindMinBucket(sPQueue32 *Q){
      // moves to next element
      while(Q->C.first[Q->C.minvalue] == NIL)
	Q->C.minvalue++;
      return Q->C.minvalue;
    }
#endif
    ...
    /// \cond
    inline int _FastFindMaxBucket(sPQueue32 *Q){
      int current;
    ...
    int    FastGetMinFIFO(sPQueue32 *Q){
      int bucket;
      bucket = _FastFindMinBucket(Q);
      return Q->C.first[bucket];
    }

    int    FastGetMinVal(sPQueue32 *Q){
      return _FastFindMinBucket(Q);
    }


    int    FastGetMaxVal(sPQueue32 *Q){
      return _FastFindMaxBucket(Q);
    }
