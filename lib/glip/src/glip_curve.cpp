...
namespace glip{
  namespace Curve{
    ...
    sCurve *Create(int n){
      ...
      if(curve != NULL){
	curve->X = glip::AllocFloatArray(n);
	curve->Y = glip::AllocFloatArray(n);
	curve->n = n;
      } 
      else glip::Error((char *)MSG1,
		      (char *)"Curve::Create");
      return(curve);
    }
    ...
    void   Destroy(sCurve **curve){
      ...
      aux = *curve;
      if(aux != NULL){
	if(aux->X != NULL) glip::FreeFloatArray(&aux->X);
	if(aux->Y != NULL) glip::FreeFloatArray(&aux->Y);
	free(aux);
	*curve = NULL;
      }
    }
