...
namespace glip{
  namespace Scene{
    ...
    sScene *Create(int xsize,int ysize,int zsize,
		   int nbits, glip_SceneType type){
      sScene *scn;
      scn = (sScene *) calloc(1,sizeof(sScene));
      if(scn == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene::Create");
	...
	case integer:
	  ...
	  if(dest->nbits>src->nbits){
	    ...
	    glip::Error((char *)"Incompatible number of bits",
		       (char *)"Scene::Copy");
	  break;
	case reals:
	  if(dest->nbits==src->nbits){
    ...
    sScene *Clone(sScene *scn){
      ...
      if(aux == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene::Clone");
    ...
    sScene *SubScene(sScene *scn,
      ...
      aux = (sScene *) calloc(1,sizeof(sScene));
      if(aux == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene::SubScene");
    ...
    sScene *MBB(sScene *scn){
      ...
      aux = (sScene *) calloc(1,sizeof(sScene));
      if(aux == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene::MBB");
    ...
    sScene *AddFrame(sScene *scn,  int sz, int value){
      ...
      aux = (sScene *) calloc(1,sizeof(sScene));
      if(aux == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene::AddFrame");
    ...
    sScene *RemFrame(sScene *fscn, int sz){
      ...
      aux = (sScene *) calloc(1,sizeof(sScene));
      if(aux == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene::RemFrame");
