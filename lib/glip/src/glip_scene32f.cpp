...
namespace glip{
  ...
  sScene32f *Create(int xsize,int ysize,int zsize){
    ...
    scn = (sScene32f *) calloc(1,sizeof(sScene32f));
    if(scn == NULL) 
      glip::Error((char *)MSG1,(char *)"Scene32f::Create");
    ...
    scn->data   = glip::AllocFloatArray(N);
    if(scn->data==NULL) 
      glip::Error((char *)MSG1,(char *)"Scene32f::Create");

    scn->array = (float ***) calloc(zsize, sizeof(float **));
    if(scn->array==NULL) 
      glip::Error((char *)MSG1,(char *)"Scene32f::Create");

    tmp = (float **) calloc(zsize*ysize, sizeof(float *));
    if(tmp==NULL) 
      glip::Error((char *)MSG1,(char *)"Scene32f::Create");
  ...
  void     Destroy(sScene32f **scn){
    ...
    aux = *scn;
    if(aux != NULL){
      if(aux->data     != NULL) glip::FreeFloatArray(&aux->data);
  ...
  sScene32f *Clone(sScene32f *scn){
    sScene32f *aux = Create(scn->xsize,scn->ysize,scn->zsize);
    aux->dx     = scn->dx;
    aux->dy     = scn->dy;
    aux->dz     = scn->dz;
  ...
  void     Copy(sScene32f *dest, sScene32f *src){
    if(dest->xsize!=src->xsize ||
      ...
      glip::Error((char *)"Incompatible data",(char *)"Scene32f::Copy");
    dest->dx     = src->dx;
    dest->dy     = src->dy;
    dest->dz     = src->dz;
  ...
  void     Write(sScene32f *scn, char *filename){
    ...
    if(fp == NULL) 
      glip::Error((char *)MSG2,(char *)"Scene32f::Write");
