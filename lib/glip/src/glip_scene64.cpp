...
namespace glip{
  namespace Scene64{
    ...
    sScene64 *Create(int xsize,int ysize,int zsize){
      ...
      scn = (sScene64 *) calloc(1,sizeof(sScene64));
      if(scn == NULL) 
	glip::Error((char *)MSG1,(char *)"Scene64::Create");
      ...
      scn->data   = glip::AllocLongLongArray(N);
      if(scn->data==NULL) 
	glip::Error((char *)MSG1,(char *)"Scene64::Create");
	...
	*scn = NULL;
    ...
    sScene64 *Clone(sScene64 *scn){
      sScene64 *aux = Create(scn->xsize,scn->ysize,scn->zsize);
      aux->dx     = scn->dx;
      aux->dy     = scn->dy;
      aux->dz     = scn->dz;
	...
	glip::Error((char *)"Incompatible data",(char *)"Scene64::Copy");
      dest->dx     = src->dx;
      dest->dy     = src->dy;
      dest->dz     = src->dz;
