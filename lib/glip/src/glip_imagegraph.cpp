...
namespace glip{
  namespace ImageGraph{
    ...
    sImageGraph *ByEuclideanDistance(sImage32 *img, float r, int d){
      ...
      for(p=0; p<n; p++){
	p_x = p%img->ncols; //PixelX(img,p);
	p_y = p/img->ncols; //PixelY(img,p);
	
	(sg->n_link[p])[0] = NIL;
	for(i=1; i<A->n; i++){
	  q_x = p_x + A->dx[i];
	  q_y = p_y + A->dy[i];
	  if(!Image32::IsValidPixel(img,q_x,q_y)){
	    (sg->n_link[p])[i] = INT_MAX;
	    continue;
	  }
	  q = q_x + img->ncols*q_y;
	  ...
	  qq_x = q_x + A->dx[i]*d;
	  qq_y = q_y + A->dy[i]*d;
	  if(Image32::IsValidPixel(img,qq_x,qq_y)){
	    qq = qq_x + img->ncols*qq_y;
	    v1 = img->data[qq];
	  }
	  else
	    v1 = img->data[q];
	  ...
	  pp_x = p_x - A->dx[i]*d;
	  pp_y = p_y - A->dy[i]*d;
	  if(Image32::IsValidPixel(img,pp_x,pp_y)){
	    pp = pp_x + img->ncols*pp_y;
	    v2 = img->data[pp];
	  }
	  else
	    v2 = img->data[p];
    ...
    sImageGraph *ByEuclideanDistance(sCImage *cimg, float r, int d){
      ...
      int p_x,p_y,q_x,q_y;
      int pp_x,pp_y,qq_x,qq_y;
      int dr,dg,db,r1,g1,b1,r2,g2,b2;
      int ncols, nrows;
      ncols = cimg->C[0]->ncols;
      nrows = cimg->C[0]->nrows;
      n  = ncols*nrows;
      ...
      for(p=0; p<n; p++){
	p_x = p%ncols; //PixelX(img,p);
	p_y = p/ncols; //PixelY(img,p);
	
	(sg->n_link[p])[0] = NIL;
	for(i=1; i<A->n; i++){
	  q_x = p_x + A->dx[i];
	  q_y = p_y + A->dy[i];
	  if(!Image32::IsValidPixel(cimg->C[0], q_x, q_y)){
	    (sg->n_link[p])[i] = INT_MAX;
	    continue;
	  }
	  q = q_x + ncols*q_y;
	  ...
	  qq_x = q_x + A->dx[i]*d;
	  qq_y = q_y + A->dy[i]*d;
	  if(Image32::IsValidPixel(cimg->C[0], qq_x, qq_y)){
	    qq = qq_x + ncols*qq_y;
	    r1 = cimg->C[0]->data[qq];
	    g1 = cimg->C[1]->data[qq];
	    b1 = cimg->C[2]->data[qq];
	  ...
	  pp_x = p_x - A->dx[i]*d;
	  pp_y = p_y - A->dy[i]*d;
	  if(Image32::IsValidPixel(cimg->C[0], pp_x, pp_y)){
	    pp = pp_x + ncols*pp_y;
	    r2 = cimg->C[0]->data[pp];
	    g2 = cimg->C[1]->data[pp];
	    b2 = cimg->C[2]->data[pp];
    ...
    sImageGraph *ByEuclideanDistance(sCImage32f *cimg, float r, int d){
      ...
      int p_x,p_y,q_x,q_y;
      int pp_x,pp_y,qq_x,qq_y;
      float dr,dg,db,r1,g1,b1,r2,g2,b2;
      int ncols, nrows;
      ncols = cimg->C[0]->ncols;
      nrows = cimg->C[0]->nrows;
      n  = ncols*nrows;
	...
	for(i=1; i<A->n; i++){
	  q_x = p_x + A->dx[i];
	  q_y = p_y + A->dy[i];
	  if(!Image32f::IsValidPixel(cimg->C[0], q_x, q_y)){
	      (sg->n_link[p])[i] = INT_MAX;
	      continue;
	  }
	  q = q_x + ncols*q_y;
	  ...
	  qq_x = q_x + A->dx[i]*d;
	  qq_y = q_y + A->dy[i]*d;
	  if(Image32f::IsValidPixel(cimg->C[0], qq_x, qq_y)){
	    qq = qq_x + ncols*qq_y;
	    r1 = cimg->C[0]->data[qq];
	    g1 = cimg->C[1]->data[qq];
	    b1 = cimg->C[2]->data[qq];
	  ...
	  pp_x = p_x - A->dx[i]*d;
	  pp_y = p_y - A->dy[i]*d;
	  if(Image32f::IsValidPixel(cimg->C[0], pp_x, pp_y)){
	    pp = pp_x + ncols*pp_y;
	    r2 = cimg->C[0]->data[pp];
	    g2 = cimg->C[1]->data[pp];
	    b2 = cimg->C[2]->data[pp];
    ...
    sImageGraph *ByPixelEuclideanDistance(sImage32 *img, float r, int f){
      ...
      D = (int *)calloc(A->n, sizeof(int));
      for(i=1; i<A->n; i++){
	D[i] = ROUND(sqrtf(A->dx[i]*A->dx[i] + A->dy[i]*A->dy[i])*f);
      }
	...
	for(i=1; i<A->n; i++){
	  v_x = u_x + A->dx[i];
	  v_y = u_y + A->dy[i];
	  if(Image32f::IsValidPixel(W,v_x,v_y)){
	    q = v_x + W->ncols*v_y;
	    
	    weight = ROUND(W->data[p] + W->data[q]) + 1;
	    (sg->n_link[p])[i] = weight;
    ...
    sImageGraph   *ByHomogeneityAffinity(sImage32 *img, float r){
      sImageGraph *sg=NULL;
      sAdjRel *A = glip::AdjRel::Circular(r);
      int p,q,i,n;
      int weight;
      Pixel u,v;
	...
	for(i=1; i<A->n; i++){
	  v.x = u.x + A->dx[i];
	  v.y = u.y + A->dy[i];
	  if(glip::Image32::IsValidPixel(img,v.x,v.y)){
	    q = v.x + img->ncols * v.y;
    ...
    */
	...
	for(i=1; i<A->n; i++){
	  v.x = u.x + A->dx[i];
	  v.y = u.y + A->dy[i];
	  if(glip::Image32::IsValidPixel(cimg->C[0],v.x,v.y)){
	    q = v.x + ncols*v.y;
	    dr = abs(cimg->C[0]->data[p] - cimg->C[0]->data[q]);
	    dg = abs(cimg->C[1]->data[p] - cimg->C[1]->data[q]);
	    db = abs(cimg->C[2]->data[p] - cimg->C[2]->data[q]);
    ...
    */
	...
	for(i=1; i<A->n; i++){
	  v.x = u.x + A->dx[i];
	  v.y = u.y + A->dy[i];
	  if(glip::Image32::IsValidPixel(obj,v.x,v.y)){
	    q = v.x + obj->ncols * v.y;
	...
	for(i = 1; i < A->n; i++){
	  v.x = u.x + A->dx[i];
	  v.y = u.y + A->dy[i];
	  if(glip::Image32::IsValidPixel(Pobj,v.x,v.y)){
    ...
    int GetNumberOfEdges(sImageGraph *graph){
      //It assumes a symmetric adjacency relation.
      int i,p,q,n;
      int ncols, nrows, nedges;
      int u_x,u_y,v_x,v_y;
      sAdjRel *A;
      ncols = graph->ncols;
      nrows = graph->nrows;
    ...
    sImageGraph   *Create(int ncols, int nrows, sAdjRel *A){
      ...
      if(sg == NULL)
	glip::Error((char *)MSG1,(char *)"ImageGraph::Create");
      sg->A = A;
      sg->type = DISSIMILARITY;
      sg->ncols = ncols;
      sg->nrows = nrows;
	...
	glip::Error((char *)MSG1,(char *)"ImageGraph::Create");
      for(p=0; p<n; p++){
	sg->n_link[p] = glip::AllocIntArray(A->n);
	for(i=0; i<A->n; i++)
	  (sg->n_link[p])[i] = INT_MAX;
      }
      sg->Wmax = 0;
	...
	*sg = NULL;
    ...
    sImageGraph *ReadFromTxt(char *filename){
      ...
      fp = fopen(filename,"r");
      if(fp == NULL)
	glip::Error((char *)"Cannot open file",
    ...
    void         Write2Txt(sImageGraph *sg, 
      ...
      if(sg->A->n!=5)
	glip::Error((char *)"Current implementation only supports adjacency-4",
...
namespace glip{
  namespace CImage{
    ...
    sCImage *ArcWeightImage(sImageGraph *sg){
      int i,im,jm,p,n,weight,dx,dy,val,val_inv;
      int q,qx,qy;
      float theta;
      sCImage *arcw;
      sAdjRel *A;
      ...
      A    = sg->A;
      arcw = CImage::Create(sg->ncols,sg->nrows);
      n    = sg->ncols*sg->nrows;
      i_inv = glip::AdjRel::InverseIndexes(A);
      
      if(sg->type == CAPACITY){
	for(p=0; p<n; p++){
	  arcw->C[0]->data[p] = INT_MAX;
	  im = 0;
	  for(i=1; i<A->n; i++){
	    weight = (sg->n_link[p])[i];
	    if(weight == 0) continue;
	    if(weight < arcw->C[0]->data[p]){
	      arcw->C[0]->data[p] = weight;
	      im = i;
	    }
	  }
	  arcw->C[1]->data[p] = im;
	...
	for(p=0; p<n; p++){
      ...
      if(sg->type == CAPACITY){
	for(p=0; p<n; p++){
	  val = arcw->C[0]->maxval - arcw->C[0]->data[p];
	  im = arcw->C[1]->data[p];
	  jm = i_inv[im];
	  qx = p%sg->ncols + A->dx[im];
	  qy = p/sg->ncols + A->dy[im];
	  q = qx + sg->ncols*qy;
	  val_inv = arcw->C[0]->maxval - (sg->n_link[q])[jm];
	  dx =  A->dx[im];
	  dy = -A->dy[im];
	  theta = atan2f(dy, dx); //in radians in the range [-pi, pi].
	  if(theta < 0.0)
	    theta += 2.0*PI;
	  hsv.h = (theta*180)/PI;
	  hsv.s = (255*(val-val_inv))/val;
	  hsv.v = (255*val)/arcw->C[0]->maxval;
	  rgb = glip::Color::HSV2RGB(hsv);
	  arcw->C[0]->data[p] = rgb.r;
	  arcw->C[1]->data[p] = rgb.g;
	  arcw->C[2]->data[p] = rgb.b;
	}
      }
      else{ //DISSIMILARITY
	for(p=0; p<n; p++){
