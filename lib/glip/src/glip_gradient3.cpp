...
namespace glip{
  namespace Gradient3{
    ...
    sGradient3 *Create(int xsize,int ysize,int zsize){
      sGradient3 *grad=NULL;
      grad = (sGradient3 *) calloc(1,sizeof(sGradient3));
      if(grad == NULL)
	glip::Error((char *)MSG1,
		   (char *)"Gradient3::Create");
      grad->Gx = Scene32::Create(xsize,ysize,zsize);
      grad->Gy = Scene32::Create(xsize,ysize,zsize);
      grad->Gz = Scene32::Create(xsize,ysize,zsize);
      grad->mag = NULL;
      return grad;
    }
	...
	*grad = NULL;
	...
	glip::Error((char *)MSG1,
      ...
      grad->Gx = Scene32::RemFrame(fgrad->Gx, sz);
      grad->Gy = Scene32::RemFrame(fgrad->Gy, sz);
      grad->Gz = Scene32::RemFrame(fgrad->Gz, sz);
      if(fgrad->mag!=NULL){
	grad->mag = Scene32::RemFrame(fgrad->mag, sz);
    ...
    sGradient3 *LinearInterpCentr(sGradient3 *grad,
      ...
      sGradient3 *interp=NULL;
      interp = (sGradient3 *) calloc(1,sizeof(sGradient3));
      if(interp == NULL)
	glip::Error((char *)MSG1,
    ...
    */
    ...
    sGradient3 *ChangeOrientationToLPS(sGradient3 *grad,
      ...
      lps = (sGradient3 *) calloc(1,sizeof(sGradient3));
      if(lps == NULL)
	glip::Error((char *)MSG1,
      ...
      else if(ori[0]=='R'){ lps->Gx = gx; Scene32::Negateinplace(gx); }
      else if(ori[0]=='P'){ lps->Gy = gx; }
      else if(ori[0]=='A'){ lps->Gy = gx; Scene32::Negateinplace(gx); }
      else if(ori[0]=='S'){ lps->Gz = gx; }
      else if(ori[0]=='I'){ lps->Gz = gx; Scene32::Negateinplace(gx); }
      else{ glip::Error((char *)"Invalid orientation",
		       (char *)"Gradient3::ChangeOrientationToLPS"); }
      
      if     (ori[1]=='L'){ lps->Gx = gy; }
      else if(ori[1]=='R'){ lps->Gx = gy; Scene32::Negateinplace(gy); }
      else if(ori[1]=='P'){ lps->Gy = gy; }
      else if(ori[1]=='A'){ lps->Gy = gy; Scene32::Negateinplace(gy); }
      else if(ori[1]=='S'){ lps->Gz = gy; }
      else if(ori[1]=='I'){ lps->Gz = gy; Scene32::Negateinplace(gy); }
      else{ glip::Error((char *)"Invalid orientation",
		       (char *)"Gradient3::ChangeOrientationToLPS"); }
      
      if     (ori[2]=='L'){ lps->Gx = gz; }
      else if(ori[2]=='R'){ lps->Gx = gz; Scene32::Negateinplace(gz); }
      else if(ori[2]=='P'){ lps->Gy = gz; }
      else if(ori[2]=='A'){ lps->Gy = gz; Scene32::Negateinplace(gz); }
      else if(ori[2]=='S'){ lps->Gz = gz; }
      else if(ori[2]=='I'){ lps->Gz = gz; Scene32::Negateinplace(gz); }
      else{ glip::Error((char *)"Invalid orientation",
		       (char *)"Gradient3::ChangeOrientationToLPS"); }
      
      if(lps->Gx==NULL || lps->Gy==NULL || lps->Gz==NULL)
	glip::Error((char *)"Invalid orientation",
		   (char *)"Gradient3::ChangeOrientationToLPS");
	...
	m  = (grad->mag)->data[p];
	nm = (float)glip::LinearStretch((grad->mag)->data[p],
				       omin, omax, 
				       nmin, nmax);
	gx = grad->Gx->data[p];
	gy = grad->Gy->data[p];
	...
	if((grad->mag)->data[p]>0){
      ...
      (grad->mag)->maxval = glip::LinearStretch((grad->mag)->maxval,
					       omin, omax, 
					       nmin, nmax);
    }
