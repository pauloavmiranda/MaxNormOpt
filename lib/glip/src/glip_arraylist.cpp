  ...
  namespace ArrayList{
	...
	*mem=NULL;
    ...
    sArrayList *Create(int cap){
      ...
      A = (sArrayList *) calloc(1,sizeof(sArrayList));
      if(A == NULL)
	glip::Error((char *)MSG1,(char *)"ArrayList::Create");
	...
	*A = NULL;
	...
	A->array = (void **)realloc(A->array,
				    A->cap*sizeof(void *));
	if(A->array == NULL)
	  glip::Error((char *)MSG1,(char *)"ArrayList::AddElement");
	for(i=A->n; i<A->cap; i++)
	  A->array[i] = NULL;
	AddElement(A, elem);
      }
	...
	if(A->array[i]!=NULL)
      ...
      A->array = (void **)realloc(A->array,
				  n*sizeof(void *));
      if(A->array == NULL)
	glip::Error((char *)MSG1,(char *)"ArrayList::Resize");
      for(i=A->n; i<A->cap; i++)
	A->array[i] = NULL;
    }
