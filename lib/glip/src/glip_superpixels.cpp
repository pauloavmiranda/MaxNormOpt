...
namespace glip{
  namespace Superpixels{
	...
	if (Q->cost[p] != FLT_MAX) {
	  if (Q->color[p] != BLACK) {
	    glip::Heap::Update_MinPolicy(Q, p, cost[p]);
	  } else {
	...
	for(x = 0; x < xsize; x++){
	  y = 0;
	  p = glip::Scene32::GetVoxelAddress(fscn, x, y, z);
	  Q->color[p] = BLACK;
	  label->data[p] = -2;
	  y = ysize-1;
	  p = glip::Scene32::GetVoxelAddress(fscn, x, y, z);
	...
	*nseeds = xstrips*ystrips*zstrips;
    ...
    void UpdateSVoxelsSeeds(sScene32 *scn,
			    ...
			    sSVoxelSeed *seeds, int nseeds,
			    int inside){
      int s,p,q,t,N,i,j,k,ci,cj,ck,cd,d;
      sQueue *Q;
	...
	if(seeds[s].updateseeds){
	  ...
	  for(s = 0; s < nseeds; s++){
	    if(!seeds[s].updateseeds)
	      continue;
	    p = glip::Scene32::GetVoxelAddress(scn,
					      seeds[s].actual.j,
					      seeds[s].actual.i,
					      seeds[s].actual.k);
	    if(label->data[p] != s){
	      ...
	      p = glip::Scene32::GetVoxelAddress(scn, cj, ci, ck);
	      Queue::Push(Q, p);
	      while(!Queue::IsEmpty(Q)){
		p = Queue::Pop(Q);
		for(t = 1; t < V->n; t++){
