...
namespace glip{
  namespace AdjRel{
    ...
    sAdjRel *Create(int n){
      ...
      if (A != NULL){
	A->dx = glip::AllocIntArray(n);
	A->dy = glip::AllocIntArray(n);
	A->n  = n;
      } else {
	glip::Error((char *)MSG1,(char *)"AdjRel::Create");
      }
    ...
    void Destroy(sAdjRel **A){
      ...
      aux = *A;
      if (aux != NULL){
	if (aux->dx != NULL) glip::FreeIntArray(&(aux->dx));//free(aux->dx);
	if (aux->dy != NULL) glip::FreeIntArray(&(aux->dy));//free(aux->dy);
	free(aux);
	*A = NULL;
      }   
    }
    ...
    sAdjRel *Neighborhood_8_counterclockwise(){ /* 8-neighborhood */
      sAdjRel *A=NULL;
      A = Create(8+1);
      /* place central pixel at first */
      A->dx[0] =  0; A->dy[0] =  0;
    ...
    sAdjRel *Neighborhood_8_clockwise(){ /* 8-neighborhood */
      sAdjRel *A=NULL;
      A = Create(8+1);
      /* place central pixel at first */
      A->dx[0] =  0; A->dy[0] =  0;
    ...
    int *InverseIndexes(sAdjRel *A){
      int *i_inv;
      int i,j;
      i_inv = (int *)malloc(sizeof(int)*A->n);
      i_inv[0] = 0;
      for(i=1; i<A->n; i++){
	i_inv[i] = NIL;
	for(j=1; j<A->n; j++){
    ...
    sAdjPxl  *CreateAdjPxl(int n){
      ...
      N = (sAdjPxl *)calloc(1, sizeof(sAdjPxl));
      if(N == NULL)
	Error((char *)MSG1,(char *)"AdjRel::CreateAdjPxl");
      N->dp = glip::AllocIntArray(n);
      N->n = n;
      return N;
    }
    ...
    void DestroyAdjPxl(sAdjPxl **N){
      ...
      aux = *N;
      if (aux != NULL){
	if (aux->dp != NULL) glip::FreeIntArray(&aux->dp);
	free(aux);
	*N = NULL;
      }
    }
    ...
    void Reduce2Boundary(sAdjRel **A, float r){
      ...
      int n, p, k;
      if(A == NULL) return;
      AC = glip::AdjRel::Circular(r);
      I = glip::Image32::Render(*A);
      B = glip::Image32::GetObjBorders(I, AC, true);
      //glip::Image32::Write(I, (char *)"I.pgm");
      //glip::Image32::Write(B, (char *)"B.pgm");
      n = 0;
      for(p = 0; p < I->n; p++){
	if(B->data[p] > 0)
	  n++;
      }
      AB = glip::AdjRel::Create(n);
      //AB->dx[0] = (*A)->dx[0];
      //AB->dy[0] = (*A)->dy[0];
      //k = 1;
      k = 0;
      for(p = 0; p < I->n; p++){
	if(B->data[p] > 0){
	  AB->dx[k] = p%I->ncols - I->ncols/2;
	  AB->dy[k] = p/I->ncols - I->nrows/2;
	  k++;
	}
      }
      ...
      *A = AB;
      ...
      glip::Image32::Destroy(&B);
      glip::AdjRel::Destroy(&AC);
    }
...
namespace glip{
  namespace CImage{
	...
	if(glip::Image32::IsValidPixel(cimg->C[0], v.x,v.y)){
	  cimg->C[0]->array[v.y][v.x] = glip::Color::Channel0(color);
	  cimg->C[1]->array[v.y][v.x] = glip::Color::Channel1(color);
	  cimg->C[2]->array[v.y][v.x] = glip::Color::Channel2(color);
	}
      }
    }
