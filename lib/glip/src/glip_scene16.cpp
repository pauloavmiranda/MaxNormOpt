...
namespace glip{
  ...
  sScene16 *Create(int xsize,int ysize,int zsize){
    ...
    scn = (sScene16 *) calloc(1,sizeof(sScene16));
    if(scn == NULL) 
    ...
    scn->data   = glip::AllocUShortArray(N);
    if(scn->data==NULL) 
      Error((char *)MSG1,(char *)"Scene16::Create");
  ...
  void     Destroy(sScene16 **scn){
    ...
    aux = *scn;
    if(aux != NULL){
      if(aux->data     != NULL) glip::FreeUShortArray(&aux->data);
  ...
  sScene16 *Clone(sScene16 *scn){
    sScene16 *aux = Create(scn->xsize,scn->ysize,scn->zsize);
    aux->dx     = scn->dx;
    aux->dy     = scn->dy;
    aux->dz     = scn->dz;
