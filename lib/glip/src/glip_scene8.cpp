...
namespace glip{
  ...
  sScene8 *Create(int xsize,int ysize,int zsize){
    ...
    scn = (sScene8 *) calloc(1,sizeof(sScene8));
    if(scn == NULL) 
    ...
    scn->data   = glip::AllocUCharArray(N);
    if(scn->data==NULL) 
      Error((char *)MSG1,(char *)"Scene8::Create");
  ...
  void     Destroy(sScene8 **scn){
    ...
    aux = *scn;
    if(aux != NULL){
      if(aux->data     != NULL) glip::FreeUCharArray(&aux->data);
  ...
  sScene8 *Clone(sScene8 *scn){
    sScene8 *aux = Create(scn->xsize,scn->ysize,scn->zsize);
    aux->dx     = scn->dx;
    aux->dy     = scn->dy;
    aux->dz     = scn->dz;
  ...
  uchar   GetMaxVal(sScene8 *scn){
    return GetMaximumValue(scn);
  }
    
  uchar   GetMinVal(sScene8 *scn){
