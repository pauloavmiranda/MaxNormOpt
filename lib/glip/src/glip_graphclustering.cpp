...
namespace glip{
  namespace Graph{
    ...
    int *DivisiveClusteringByMST(sGraph *G, int c){
      ...
      S[0] = 1;
      S[1] = 0;
      label = glip::AllocIntArray(G->nnodes);
      pred  = glip::AllocIntArray(G->nnodes);
      cost  = glip::AllocIntArray(G->nnodes);
      for(p = 0; p < G->nnodes; p++)
	label[p] = NIL;
      label[0] = 0;
      glip::ift::IFT_fw(G, S, label, cost, pred);
	...
	for(i = 0; i < G->nodes[p].outdegree; i++){
      ...
      Queue::Destroy(&F);
      glip::FreeIntArray(&pred);
      glip::FreeIntArray(&cost);
      return label;
    }
	...
	for(i = 0; i < G->nodes[p].outdegree; i++){
	  q = G->nodes[p].adjList[i];
	  w = G->nodes[p].Warcs[i];
	  if(w != NIL && w < energy && glip::BMap::Get(B, q)==0){
	    glip::Queue::Push(Q, q);
	    glip::BMap::Set1(B, q);
	    size++;  
	  }
    ...
    glip::sImage32 *ComputeEnegyMap_UOIFT(glip::sImage32 *spixels, sGraph *G){
      ...
      S[1] = 0;
      nnodes = G->nnodes;
      energy = glip::Image32::Create(spixels);
      label = glip::AllocIntArray(nnodes);
      cost  = glip::AllocIntArray(nnodes);
      for(p = 0; p < nnodes; p++)
	label[p] = NIL;
      label[0] = 0;
      T = Transpose(G);
	...
	lb = label[p];
	oift_label = glip::AllocIntArray(tree[lb].G->nnodes);
	...
	i++;
      ...
      for(i = 0; i < c*2; i++){
	if(tree[i].G != NULL)
	  glip::Graph::Destroy(&tree[i].G);
      }
      free(tree);
      glip::FreeIntArray(&cost);

      //Fixing the labels to match the number of partitions:
      hist = glip::AllocIntArray(n);
      for(p = 0; p < nnodes; p++){
	hist[label[p]] += 1;
      }
      mapping = glip::AllocIntArray(n);
      i = 0;
      for(l = 0; l < n; l++){
	if(hist[l] > 0){
	  mapping[l] = i;
	...
	tree[i].left = NIL;
      ...
      label = glip::AllocIntArray(nnodes);
      cost  = glip::AllocIntArray(nnodes);
      DCC_size = glip::AllocIntArray(nnodes);
      for(p = 0; p < nnodes; p++)
	label[p] = NIL;
      label[0] = 0;
      T = Transpose(G);
	...
	*rp = rq;
	...
	Error(MSG1, "glip::Graph::ClusteringByMST2");
      //--------------------------------
