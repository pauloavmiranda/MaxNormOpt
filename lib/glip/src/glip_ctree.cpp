...
namespace glip{
  namespace CTree{
    ...
    sCTree *Create(int size){
      ...
      if(ct == NULL)
	glip::Error((char *)MSG1,(char *)"CTree::Create");
	...
	glip::Error((char *)MSG1,(char *)"Graph::EdgeBasedMinTree");
    ...
    void ComputeHeight(sCTree *ct){
      ...
      glip::sStack *S;
      glip::sQueue *Q;
      Q = glip::Queue::Create(ct->nnodes);
      S = glip::Stack::Create(ct->nnodes);
      for(i = 0; i < ct->nnodes; i++)
	if(ct->nodes[i].parent == NULL)
	  root = i;

      //Reverse Breadth-first search:
      glip::Queue::Push(Q, root);
