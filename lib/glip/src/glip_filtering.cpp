...
namespace glip{
  ...
  namespace Kernel{
    ...
    sKernel *Create(sAdjRel *A){
      ...
      if(K == NULL){
	glip::Error(MSG1, "Kernel::Create");
      }
      K->val = glip::AllocFloatArray(A->n);
    ...
    sKernel *Clone(sKernel *K){
      ...
      if(C == NULL)
	glip::Error(MSG1, "Kernel::Clone");
      
      C->val = glip::AllocFloatArray(K->adj->n);
      memcpy(C->val, K->val,
    ...
    void    Destroy(sKernel **K){
      ...
      aux = *K;
      if(aux != NULL){
	if (aux->val != NULL) glip::FreeFloatArray(&(aux->val));
    ...
    */
	...
	for(u_x = 0; u_x < img->ncols; ++u_x){
	  ...
	  for(i = 0; i < K->adj->n; ++i){
	    ...
	    //------------28/05/26------------------------------
	    else //repeats central value for neighbors beyond borders.
	      conv += (float)img->array[u_y][u_x] * K->val[i];
	    //--------------------------------------------------
	  }
  ...
  namespace CImage{
    ...
    sImage32 *SobelFilter(sCImage *cimg){
      ...
      int p;
      g0 = glip::Image32::SobelFilter(cimg->C[0]);
      g1 = glip::Image32::SobelFilter(cimg->C[1]);
      g2 = glip::Image32::SobelFilter(cimg->C[2]);
      grad = glip::Image32::Create(cimg->C[0]);
	...
	*/
	grad->data[p] = ROUND(sqrtf(g0->data[p]*g0->data[p] +
      ...
      glip::Image32::Destroy(&g0);
      glip::Image32::Destroy(&g1);
      glip::Image32::Destroy(&g2);
      return grad;
    }


    sCImage *GaussianBlur(sCImage *cimg){
      sCImage *cblur;
      sKernel *K,*N;
      cblur = glip::CImage::Create(cimg);
      K = Kernel::Make("5,5, 1, 4, 7, 4, 1, 4,16,26,16, 4, 7,26,41,26, 7, 4,16,26,16, 4, 1, 4, 7, 4, 1");
      N = Kernel::Normalize(K);
      glip::Image32::Destroy( &cblur->C[0] );
      cblur->C[0] = glip::Image32::LinearFilter(cimg->C[0], N);
      glip::Image32::Destroy( &cblur->C[1] );
      cblur->C[1] = glip::Image32::LinearFilter(cimg->C[1], N);
      glip::Image32::Destroy( &cblur->C[2] );
      cblur->C[2] = glip::Image32::LinearFilter(cimg->C[2], N);
      Kernel::Destroy(&K);
      Kernel::Destroy(&N);
      return cblur;
    }
  ...
  namespace CImage32f{
    ...
    sImage32f *SobelFilter(sCImage32f *cimg){
      ...
      int p;
      g0 = glip::Image32f::SobelFilter(cimg->C[0]);
      g1 = glip::Image32f::SobelFilter(cimg->C[1]);
      g2 = glip::Image32f::SobelFilter(cimg->C[2]);
      grad = glip::Image32f::Create(cimg->C[0]);
      for(p = 0; p < cimg->C[0]->n; p++){
	/*
	grad->data[p] = MAX(g0->data[p],
			    MAX(g1->data[p], g2->data[p]));
	*/
	grad->data[p] = sqrtf(g0->data[p]*g0->data[p] +
      ...
      glip::Image32f::Destroy(&g0);
      glip::Image32f::Destroy(&g1);
      glip::Image32f::Destroy(&g2);
      return grad;
    }


    sCImage32f *GaussianBlur(sCImage32f *cimg){
      sCImage32f *cblur;
      sKernel *K,*N;
      cblur = glip::CImage32f::Create(cimg);
      K = Kernel::Make("5,5, 1, 4, 7, 4, 1, 4,16,26,16, 4, 7,26,41,26, 7, 4,16,26,16, 4, 1, 4, 7, 4, 1");
      N = Kernel::Normalize(K);
      glip::Image32f::Destroy( &cblur->C[0] );
      cblur->C[0] = glip::Image32f::LinearFilter(cimg->C[0], N);
      glip::Image32f::Destroy( &cblur->C[1] );
      cblur->C[1] = glip::Image32f::LinearFilter(cimg->C[1], N);
      glip::Image32f::Destroy( &cblur->C[2] );
      cblur->C[2] = glip::Image32f::LinearFilter(cimg->C[2], N);
      Kernel::Destroy(&K);
      Kernel::Destroy(&N);
      return cblur;
    }
