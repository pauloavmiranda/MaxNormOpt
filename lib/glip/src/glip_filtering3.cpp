...
namespace glip{
	...
	K = Create(A);
	for (i=0;i<A->n;i++) {
	  d_x = (A->d[i].axis.x*scn->dx)/dmin;
	  d_y = (A->d[i].axis.y*scn->dy)/dmin;
	  d_z = (A->d[i].axis.z*scn->dz)/dmin;
	  r2 = (d_x * d_x +
		d_y * d_y +
		d_z * d_z);
	  K->val[i] = s*exp (-f*(r2/R2));
  ...
  namespace Scene32{
	...
	for(i = 1; i < A->n; i++){
	  ...
	  if(glip::Scene32::IsValidVoxel(scn, u)){
	    q = glip::Scene32::GetVoxelAddress(scn, u);
    ...
    sScene32   *GaussianBlur(sScene32 *scn){
      sScene32 *blur;
      sKernel3 *K,*NK;
      K  = Kernel3::SphericalGaussian(scn, 1.0, 10.0, 1.0); //2.0, 10.0, 1.0
    ...
    */
	...
	acc = 0;
	for(m=0; m<9; m++)
	  acc += we[m] * (scn->data[ i+dx+dz*n0[m]+dy*n1[m] ] -
			  scn->data[ i-dx+dz*n0[m]+dy*n1[m] ]);
	acc >>= 4;
	grad->data[i] = acc;
      }
	...
	acc = 0;
	for(m=0; m<9; m++)
	  acc += we[m] * (scn->data[ i+dy+dx*n0[m]+dz*n1[m] ] -
			  scn->data[ i-dy+dx*n0[m]+dz*n1[m] ]);
	acc >>= 4;
	grad->data[i] = acc;
      }
	...
	acc = 0;
	for(m=0; m<9; m++)
	  acc += we[m] * (scn->data[ i+dz+dx*n0[m]+dy*n1[m] ] -
			  scn->data[ i-dz+dx*n0[m]+dy*n1[m] ]);
	acc >>= 4;
	grad->data[i] = acc;
      }
