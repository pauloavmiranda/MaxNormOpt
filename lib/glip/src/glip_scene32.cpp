...
namespace glip{
  ...
  sScene32 *Create(int xsize,int ysize,int zsize){
    ...
    if(scn == NULL) 
      glip::Error((char *)MSG1,(char *)"Scene32::Create");
  ...
  void     Copy(sScene32 *dest, sScene32 *src){
    if(dest->xsize!=src->xsize ||
      ...
      glip::Error((char *)"Incompatible data",(char *)"Scene32::Copy");
    dest->dx     = src->dx;
    dest->dy     = src->dy;
    dest->dz     = src->dz;
  ...
  void     FlipX(sScene32 *scn){
    int i, j, jj, k, tmp;
    float dx, dy, dz;
    for(j = 0; j < scn->xsize/2; j++){
      jj = scn->xsize-1 - j;
      for(i = 0; i < scn->ysize; i++){
	for(k = 0; k < scn->zsize; k++){
	  tmp = scn->array[k][i][j];
	  scn->array[k][i][j] = scn->array[k][i][jj];
	  scn->array[k][i][jj] = tmp;
	}
      }
    }
    if(scn->nii_hdr != NULL){
	...
	for(j = 0; j < 4; j++)
	  scn->nii_hdr->sto_xyz.m[0][j] = -scn->nii_hdr->sto_xyz.m[0][j];
	scn->nii_hdr->sto_ijk = nifti_mat44_inverse( scn->nii_hdr->sto_xyz );
      }
      if(scn->nii_hdr->qform_code > 0){
	for(j = 0; j < 4; j++)
	  scn->nii_hdr->qto_xyz.m[0][j] = -scn->nii_hdr->qto_xyz.m[0][j];
	scn->nii_hdr->qto_ijk = nifti_mat44_inverse( scn->nii_hdr->qto_xyz );
	nifti_mat44_to_quatern( scn->nii_hdr->qto_xyz,
				&(scn->nii_hdr->quatern_b),
				&(scn->nii_hdr->quatern_c),
				&(scn->nii_hdr->quatern_d),
				&(scn->nii_hdr->qoffset_x),
				&(scn->nii_hdr->qoffset_y),
				&(scn->nii_hdr->qoffset_z),
				&dx, &dy, &dz,
				&(scn->nii_hdr->qfac) );
      }
    }
  ...
  void     FlipY(sScene32 *scn){
    int i, ii, j, k, tmp;
    float dx, dy, dz;
    for(i = 0; i < scn->ysize/2; i++){
      ii = scn->ysize-1 - i;
      for(j = 0; j < scn->xsize; j++){
	for(k = 0; k < scn->zsize; k++){
	  tmp = scn->array[k][i][j];
	  scn->array[k][i][j] = scn->array[k][ii][j];
	  scn->array[k][ii][j] = tmp;
	}
      }
    }
    if(scn->nii_hdr != NULL){
      if(scn->nii_hdr->sform_code > 0){
	for(j = 0; j < 4; j++)
	  scn->nii_hdr->sto_xyz.m[1][j] = -scn->nii_hdr->sto_xyz.m[1][j];
	scn->nii_hdr->sto_ijk = nifti_mat44_inverse( scn->nii_hdr->sto_xyz );
      }
      if(scn->nii_hdr->qform_code > 0){
	for(j = 0; j < 4; j++)
	  scn->nii_hdr->qto_xyz.m[1][j] = -scn->nii_hdr->qto_xyz.m[1][j];
	scn->nii_hdr->qto_ijk = nifti_mat44_inverse( scn->nii_hdr->qto_xyz );
	nifti_mat44_to_quatern( scn->nii_hdr->qto_xyz,
				&(scn->nii_hdr->quatern_b),
				&(scn->nii_hdr->quatern_c),
  ...
  void     FlipZ(sScene32 *scn){
    int i, j, k, kk, tmp;
    float dx, dy, dz;
    for(k = 0; k < scn->zsize/2; k++){
      kk = scn->zsize-1 - k;
      for(i = 0; i < scn->ysize; i++){
	for(j = 0; j < scn->xsize; j++){
	  tmp = scn->array[k][i][j];
	  scn->array[k][i][j] = scn->array[kk][i][j];
	  scn->array[kk][i][j] = tmp;
	}
      }
    }
    if(scn->nii_hdr != NULL){
      if(scn->nii_hdr->sform_code > 0){
	for(j = 0; j < 4; j++)
	  scn->nii_hdr->sto_xyz.m[2][j] = -scn->nii_hdr->sto_xyz.m[2][j];
	scn->nii_hdr->sto_ijk = nifti_mat44_inverse( scn->nii_hdr->sto_xyz );
      }
      if(scn->nii_hdr->qform_code > 0){
	for(j = 0; j < 4; j++)
	  scn->nii_hdr->qto_xyz.m[2][j] = -scn->nii_hdr->qto_xyz.m[2][j];
	scn->nii_hdr->qto_ijk = nifti_mat44_inverse( scn->nii_hdr->qto_xyz );
	nifti_mat44_to_quatern( scn->nii_hdr->qto_xyz,
				&(scn->nii_hdr->quatern_b),
				&(scn->nii_hdr->quatern_c),
  ...
  sScene32 *Read(char *filename){
    ...
    if ( (len<=4) || (strcasecmp(filename + len - 4, ".scn")!=0)) {
      glip::Error(MSG2,"Scene32::Read: Invalid file name or extension.");
      return NULL;
    }
	...
	data16 = glip::AllocUShortArray(n);
	null = fread(data16,sizeof(ushort),n,fp);
	for (i=0; i < n; i++)
	  scn->data[i] = (int) data16[i];
	glip::FreeUShortArray(&data16);
#else
	// for Motorola, IBM, SUN (BIG-ENDIAN) -> SWAp Bytes!
	glip::Warning("Data is converted from LITTLE to BIG-ENDIAN","Scene32::Read");
	data8 = glip::AllocUCharArray(2*n);
  ...
  sScene32 *ReadCompressed(char *filename) {
    ...
    b = glip_BZ2_bzReadOpen ( &bzerror, f, 0, 0, NULL, 0 );
    if ( bzerror != glip_BZ_OK ) {
      glip_BZ2_bzReadClose ( &bzerror, b );
      fclose(f);
      printf("Erro ao abrir cena compactada!\n");
      return NULL;
    }
    
    nBuf = glip_BZ2_bzRead ( &bzerror, b, buf, 1024 );
    hdr_size = 0;
    if ( ( ( bzerror == glip_BZ_OK ) || ( bzerror == glip_BZ_STREAM_END ) ) && (nBuf > 0) ) {
    ...
    nBuf = glip_BZ2_bzRead ( &bzerror, b, buf, hdr_size );
    p = 0;
    do {
      nBuf = glip_BZ2_bzRead ( &bzerror, b, buf, 1024 );
      if ( ( ( bzerror == glip_BZ_OK ) || ( bzerror == glip_BZ_STREAM_END ) ) && ( nBuf > 0 ) ) {
  ...
  void     Write(sScene32 *scn, char *filename){
    ...
    if ( (len<=4) || (strcasecmp(filename + len - 4, ".scn")!=0)) {
      glip::Error(MSG2,"Scene32::Write: Invalid file name or extension.");
    }
    ...
    if(fp == NULL) 
      glip::Error((char *)MSG2,(char *)"Scene32::Write");
  ...
  void WriteCompressed(sScene32 *scn, char *filename) {
    ...
    b = glip_BZ2_bzWriteOpen( &bzerror, f, 9, 0, 30 );
    if (bzerror != glip_BZ_OK) {
      glip_BZ2_bzWriteClose ( &bzerror, b, 0, 0, 0 );
      fclose(f);
      printf("Error on opening the file %s\n", filename);
    }
    
    //while ( (bzerror == glip_BZ_OK ) && (nBuf > 0) ) {
    if (bzerror == glip_BZ_OK) {
	...
	//printf("8bits\n");
	sprintf(data,"%s%d\n",data,8);
	glip_BZ2_bzWrite ( &bzerror, b, data, strlen( data ) );
	data8 = glip::AllocUCharArray(n);
	for (i=0; i < n; i++) 
	  data8[i] = (uchar) scn->data[i];
	glip_BZ2_bzWrite ( &bzerror, b, data8, n );
	glip::FreeUCharArray(&data8);
      } else if (Imax < 65536) {
	//printf("16bits\n");
	sprintf(data,"%s%d\n",data,16);
	...
	for (i=0; i < n; i++)
	  data16[i] = (ushort) scn->data[i];
	glip_BZ2_bzWrite ( &bzerror, b, data16, 2 * n );
	glip::FreeUShortArray(&data16);
      } else {
	//printf("32bits\n");
	sprintf(data,"%s%d\n",data,32);
	glip_BZ2_bzWrite ( &bzerror, b, data, strlen( data ) );
	glip_BZ2_bzWrite ( &bzerror, b, scn->data, 4 * n );
      }
      free(data);
	/*
	  nBuf = fread(buf, sizeof(char), 1024, in);
	  if (nBuf > 0)
	  glip_BZ2_bzWrite ( &bzerror, b, buf, nBuf );
	  
	  if (bzerror == glip_BZ_IO_ERROR) { 
	  break;
	  }
	*/
    }
  ...
  void WriteNifti1(sScene32 *scn, char *filename){
	...
	( ( float* ) nii->data )[ p ] = scn->data[ p ] / nii->flt_conv_factor;
      ...
      glip::Error( MSG5, "Scene32::WriteNifti1" );
    */
  ...
  void UpdateNiftiHeader(sScene32 *scn){
    if(scn->nii_hdr != NULL){
      scn->nii_hdr->pixdim[1] = scn->nii_hdr->dx = scn->dx;
      scn->nii_hdr->pixdim[2] = scn->nii_hdr->dy = scn->dy;
      scn->nii_hdr->pixdim[3] = scn->nii_hdr->dz = scn->dz;

      scn->nii_hdr->dim[1] = scn->nii_hdr->nx = scn->xsize;
      scn->nii_hdr->dim[2] = scn->nii_hdr->ny = scn->ysize;
      scn->nii_hdr->dim[3] = scn->nii_hdr->nz = scn->zsize;
      scn->nii_hdr->nvox = scn->n;
    }
  }
  ...
  sScene32 *ChangeOrientationToLPS(sScene32 *scn, char *ori){
    ...
    else{ glip::Error("Invalid orientation","ChangeOrientationToLPS"); }
    ...
    res= glip::Scene32::Create(xsize,ysize,zsize);
    res->dx = dx;
    res->dy = dy;
    res->dz = dz;
	...
	for (v.c.x=0; v.c.x < xsize; v.c.x++) {
	  p = glip::Scene32::GetVoxelAddress(res, v);
	  ...
	  q = glip::Scene32::GetVoxelAddress(scn, oldx, oldy, oldz);   
  ...
  sScene32 *LinearInterp(sScene32 *scn,float dx,float dy,float dz){
    int value;
    sScene32 *scene,*tmp;
    glip::Voxel P,Q,R; /* previous, current, and next voxel */
    float min=FLT_MAX;
    ...
    CloneNiftiHeader(scn, scene);
    scene->dx=dx;
    scene->dy=dy;
    scene->dz=dz;
    scene->maxval=scn->maxval;
    scene->minval=scn->minval;
    UpdateNiftiHeader(scene);
    return(scene);
  }
  ...
  sScene32 *Scale(sScene32 *scn, sScene32 *ref,
    ...
    if(scn->dx == ref->dx &&
       ...
       scn->zsize == ref->zsize){
      scl = Clone(scn);
      CloneNiftiHeader(ref, scl);
      return scl;
    }

    scl = Create(ref);
    if(interpolation == INTER_NN){ //nearest-neighbor.
	...
	for(u.c.y = 0; u.c.y < scl->ysize; u.c.y++){
      ...
      glip::Warning((char *)"This feature has not yet been implemented",
		   (char *)"Scene32::Scale");
    }
    return scl;
  }
  ...
  sScene32 *ScaleLabel(sScene32 *label, sScene32 *ref,
    ...
    else if(interpolation == INTER_NN){ //nearest-neighbor.
  ...
  sImage32 *GetSliceX(sScene32 *scn, int x){
    sImage32 *img;
    int z,y;
    img = glip::Image32::Create(scn->ysize, scn->zsize);
  ...
  sImage32 *GetSliceY(sScene32 *scn, int y){
    sImage32 *img;
    int z,x;
    img = glip::Image32::Create(scn->xsize, scn->zsize);
    for(z = 0; z < scn->zsize; z++){
  ...
  sImage32 *GetSliceZ(sScene32 *scn, int z){
    sImage32 *img;
    int *data;
    img = glip::Image32::Create(scn->xsize, scn->ysize);
