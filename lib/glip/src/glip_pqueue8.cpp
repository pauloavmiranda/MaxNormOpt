...
namespace glip{
  namespace PQueue8{
    ...
    sPQueue8 *Create(int nbuckets, int nelems, uchar *value){
      ...
      Q = (sPQueue8 *) malloc(1*sizeof(sPQueue8));
	...
	*Q = NULL;
    ...
    void Reset(sPQueue8 *Q){
      int i;
      Q->nadded = 0;
      Q->C.minvalue = UCHAR_MAX;
      Q->C.maxvalue = 0; //UCHAR_MIN;
    ...
    void   InsertElem(sPQueue8 **Q, int elem){
      ...
      if(value==UCHAR_MAX) //|| value==UCHAR_MIN)
	bucket=(*Q)->C.nbuckets;
      else{
    ...
    void   RemoveElem(sPQueue8 *Q, int elem){
      ...
      Q->nadded--;
      if ((Q->L.value[elem]==UCHAR_MAX)) //||(Q->L.value[elem]==UCHAR_MIN))
	bucket = Q->C.nbuckets;
      else
    ...
    /// \cond
    inline int _FindMinBucket(sPQueue8 *Q){
      int current,last;
    ...
    /// \cond    
    inline int _FindMaxBucket(sPQueue8 *Q){
      int current,last;
    ...
    /// \cond
    inline int _BucketFIFO(sPQueue8 *Q, int bucket){
      int elem=NIL, next;
    ...
    /// \cond
    inline int _BucketLIFO(sPQueue8 *Q, int bucket){
      int elem=NIL, prev;
    ...
    /// \cond
    inline int _FastFindMinBucket(sPQueue8 *Q){
      int current;
    ...
    /// \cond
    inline int _FastFindMaxBucket(sPQueue8 *Q){
      int current;
